...
type IngredientChoices struct {
	...
	// Describes says this declaration is the one whose presets the dropdown
	// shows, where several declarations name one dropdown. See
	// CostChoices.Describes for the whole rule; it is one rule and one field
	// written twice, because the two bindings are two types.
	Describes bool
}
...
type CostChoices struct {
	...
	// one localised_description. Several declarations may name one dropdown:
	// two recipes with IngredientsBy, or a recipe and a technology. Where
	// none of them is marked the rule is positional, recipes then
	// technologies in declaration order with the last writer winning, which
	// is what a plan that never sets this field keeps. Marking one says so
	// instead, and the settings composition, the ladder predicate, the locale
	// obligation and the game-key advisory all read the same answer.
	//
	// TWO MARKED DECLARATIONS OVER ONE DROPDOWN ARE REFUSED, and so is a
	// marked CostBy with no CostFrom beside it, which composes nothing at all
	// and would hand the dropdown an empty description while another
