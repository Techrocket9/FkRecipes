...
| `default` | the recipe exactly as the mod would have made it: the preset chosen in the dropdown beside the field where the mod declared one, and the mod's own declared list where it did not |
| `none` | no ingredients at all: a recipe that is free to craft. It also has no recycling recipe, because the game derives that one from what a recipe consumes and an empty list leaves nothing to derive it from. A science-pack list turns this word down, because a research takes at least one pack |

The setting starts out as the word `default`. Leave it there and the recipe is the mod's own: the preset chosen in the dropdown beside the field where there is one, otherwise the mod's declared list, including any fallbacks the mod declared for a modpack that lacks an ingredient. Where two of those fallbacks land on the same thing, the recipe names it once with the amounts added, and the mod's log says so. The setting's description shows that list written out, so you can copy it and change it.
...
Anything these rules turn down is reported in the log and the field behaves as though it still said `default`, so nothing you type here is the reason a load stops. The list that then decides is held to the same rules it always was: in a modpack where it cannot produce a legal recipe the load still stops, exactly as it would if you had never typed anything. The section [What happens when the text cannot be read](#what-happens-when-the-text-cannot-be-read) has the exact shape of that line.
...
- The words `default` and `none`, alone, are the recipe the mod would have made and the empty list, in any capitalisation. Neither can be combined with other entries. [Leaving the text on default](#leaving-the-text-on-default) says which list the first of them is in a given field.
...
The word `default` means whatever the mod would have done here, and it keeps meaning that when the mod changes its list in a later release: the preset chosen in the dropdown beside the field where the mod declared one, otherwise the mod's own declared list with every fallback in it. Anything else is taken as written: every name must exist in the game as loaded, and nothing is substituted. A typo is never guessed at; the text is set aside, the field behaves as though it still said `default`, and the log says what was wrong with it.

A fallback can land on something the list already carries, and the recipe that reaches the game never names one ingredient twice: the amounts are added into the earlier entry, keeping its place in the list, and a line in the log says so. For an item that line names the ingredient and both amounts; for a fluid it names the ingredient and the fallback that landed on it, so two lines about one fluid say which declaration each came from. The game refuses to load a recipe that names one ingredient twice, so this is what keeps a modpack missing one name loading at all. When the added total is above what the game can hold, 65535 for an item and 1e301 for a fluid, it is capped there with a second line, and the recipe's own tooltip says so: which fallbacks a mod set forced is not something the player chose, so the load carries on. An item and a fluid of the same name are two ingredients and are not added together. In a list you type yourself none of this applies: the same ingredient written twice is reported as you wrote it, and the field behaves as though it still said `default`.
...
**The game still loads.** A text the language cannot read is set aside: the recipe or research comes out as though the field still said `default`, which is the preset chosen in the dropdown beside the field where there is one and the mod's own declared list, every fallback in it, where there is not. One line goes into the log, and it looks like this:
...
To fix it: open Settings, then Mod settings, then the Startup tab, correct the text in the field the line names, and restart the game. Setting the field back to the word `default` is always valid and always leaves the field where a player who typed nothing leaves it: the preset chosen in the dropdown beside it where there is one, the mod's own list where there is not.

The numeric fields beside a research pack list, the count and the seconds, behave the same way: a number the game cannot use is set aside, the field decides as though it had been left alone, which beside a tier dropdown leaves the tier's own number deciding and otherwise is the mod's own default, and one line of the same shape goes to the log, ending `fix the number under Settings > Mod settings > Startup` instead. A stored value that is not a number at all is a different case and logs `the setting <name> was not readable, so its default applies`; the settings screen cannot produce one, so it comes from a file edited by hand. Nothing you type into a startup setting is the reason the mod stops loading. If what then decides cannot work in your modpack the load still stops, with the message you would have seen without typing anything, plus one sentence naming the setting whose value was set aside.
...
The library composes four or five lines onto the description, under whatever your own `[mod-setting-description]` entry says: a default line, which is the list you declared written out in the form this page documents, and then its own sentences. The ladder sentence says that an entry your mods lack takes the mod's next name for it or is left out, and that two entries landing on one name have their amounts added, so what you craft can be shorter than shown; on a science-pack setting it says the same in the packs vocabulary, about a pack and a research that can take fewer packs than shown. One shape does not carry it: an ingredient text setting with an `IngredientsBy` dropdown beside it, where the lists a player is choosing between are that dropdown's presets and the same disclosure goes onto the dropdown's own description instead. A field that stands alone carries it, and so does a science-pack field beside a `CostBy` tier, because a cost dropdown composes no such sentence of its own. The format sentence says the field takes internal names, as on the default line, up to 2000 characters. The switch sentence says which field decides while this one holds the word `default`: the dropdown above or below it when you declared one beside it, otherwise the mod's own list. The fallback sentence says that text the library cannot use is set aside as though it said `default`, with the reason in the log or the load error. On an ingredient setting the format sentence also names the word `none`, which empties the list and makes the recipe free to craft; on a science-pack setting it does not, because a pack list turns that word down. So your entry says what the setting is for, and the library says how to fill it in and what it costs to get it wrong. Where the plan should write that entry rather than the `.cfg`, because what it says depends on which mods are installed, `DescribeSetting` takes its place and the library's own lines are composed under the literal exactly as they are under the key; see [Using the library](usage.md#a-description-the-plan-writes). The entry does not have to teach the syntax:
