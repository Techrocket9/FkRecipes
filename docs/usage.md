...
A resolve-or-drop ingredient ladder that leaves the recipe with something to craft gets no such line, on purpose: it is what the ladder is for, and the library discloses it on the settings screen instead, in the ladder sentence it composes onto an ingredient dropdown's description and onto every text setting that has no dropdown beside it. That sentence names all three things a ladder does, the next name an entry offers, the entry left out where it offers none, and two entries that land on one name having their amounts added, so a player reading the field knows the list the game builds can be a shorter list than the one the tooltip shows. Where the ladders run out on EVERY entry, the sentence stops being enough and the recipe carries a line of its own, the fifth in the block above: a recipe with no ingredients costs nothing to craft, which is a balance change nobody chose rather than a shorter list. Everything else in the block is presence, arithmetic or tree shape a player cannot check anywhere: a science pack that is no longer in a price, a research that now costs nothing, a cost copied from a technology whose own price could not be read, an amount capped at a ceiling, a prerequisite dropped to open a loop, and a research that hangs off nothing because no technology it takes its cost from has a cost this library can use.

WHERE THAT SETTINGS-SCREEN SENTENCE IS COMPOSED, and where it is not: onto every ingredient dropdown, including one with no text setting beside it, whose whole composed description is your own `[mod-setting-description]` entry and that one line; and onto an ingredient or science-pack text setting that stands ALONE, whose own default line is then the list that applies. A text setting with a dropdown beside it does not carry it, because the lists the player is choosing between are that dropdown's presets and the dropdown's own description is where they are written out. A cost dropdown gets none either, because its presets render no list of internal names for a ladder to shorten, so a science-pack setting bound to a `CostFrom` beside a `CostBy` tier is the one shape where the ladder reaches no settings-screen field at all; what a copied cost's own packs do on a mod set that lacks them is disclosed where it happens, on the technology's own tooltip. A recipe declared with a plain `Ingredients` list and no setting at all has no field on the settings screen to carry it, so on that plan the ladder is disclosed to YOU, here, and nowhere a player looks. Declare an `IngredientsSetting` or an `IngredientsBy` dropdown if you want the player told; a recipe whose ladders empty it completely still says so on its own tooltip either way.
...
**Declaring your technology description in a `[technology-description]` entry keeps it, note or no note.** A prototype's own `localised_description` field wins over the locale entry of the same name, so a note written into that field on its own would stand in your sentence's place for the whole of that load. It does not. Where a technology carries a note and the plan declares no `Description`, the composition opens with a reference to that technology's own entry followed by a newline, behind an empty alternative: the engine renders your sentence and then the note where you defined the entry, and the note alone with no blank line in front of it where you did not (measured on Factorio 2.0.77, build 84539: a concatenation group holding an undefined key is itself a failed alternative, so the newline fails with it). Where the plan DOES declare `Description`, that literal is your description and takes the entry's place exactly as before, with the note under it. A prototype with no note is emitted exactly as it always was, and one with neither a note nor a declared `Description` carries no `localised_description` field at all, so the engine resolves your entry on its own.

**A recipe is composed the same way, and what a crafting tooltip does with it is not measured.** `[recipe-description]` is not where a Factorio author normally writes a recipe's description: a crafting tooltip's description comes from the RESULT ITEM, through a fallback that lives in the client's tooltip builder rather than in the prototype. Factorio 2.0.77 (build 84539) ships zero `[recipe-description]` entries in `base/locale/en/base.cfg` against 29 `[item-description]` ones, and a `localised_print` probe on a recipe prototype with only `[item-description]` defined renders `Unknown key: "recipe-description.<name>"`, so the runtime prototype value carries no item fallback of its own. The library composes the recipe key beside the technology one, so if you do write a `[recipe-description]` entry you keep it on exactly the terms above. Whether emitting a `localised_description` onto a recipe at all suppresses the client's own item-description fallback is UNMEASURED and needs a client to answer: no headless run can see it, because the dump carries the string whether or not it draws. If it does suppress it, a recipe that carries a note loses the result item's description for that load with or without this key, since the note has been emitted into that field since before the key was composed.

The key the library composes for this is your own OPTIONAL entry and the locale checker never asks for it: `CheckLocale` reports a missing `[recipe-description]` or `[technology-description]` entry as nothing at all, because requiring one would make you owe a description for every recipe and technology you emit. The one case where the key is not composed is a name too long to reference: the key is one string element and the engine caps an element at 200 bytes, so an emitted recipe name over 181 bytes or a technology name over 177 takes the note alone.
...
The dropdown's description is composed for you: your own `[mod-setting-description]` entry, then two lines per preset, its localised label and then, on a second indented line opening with `to type:`, its ingredients written out in the language, so a player about to type can start from the preset they were on; then a line saying that where an option names something the player's mods do not have, the next name that entry offers is used instead, that an entry it offers nothing for is left out, and that two entries landing on one name have their amounts added, so what they craft can be a shorter list than the one shown; and last a line saying that the text setting above or below it applies while it does not say `default` (a cost dropdown's preset stays on one line, names the technology it copies through that technology's own localised name, and carries no ladder line because it renders no list to shorten; see below). The text setting's own description carries the mirror of the switch sentence, naming the option above or below it, and carries no ladder line of its own, because this row is where the lists it would be about are. Which word each of them uses is decided by the emitted `order` strings, so the sentence is true whichever order you declared them in; the settings screen has no conditional visibility at all (measured on Factorio 2.0.77, build 84539), so those two lines are the only place the pairing can be stated.
...
**Give a dropdown option a short name, not a sentence.** Write `Express belts`, not `Express belts: the vanilla recipe with express transport belts`. The library composes the preset's typeable list beside your label, and the tooltip wraps near 57 to 60 characters (measured on Factorio 2.0.77, build 84539), so every character of the label is a character the list has to give back. A wrapped line is not lost, but it is not marked either: the engine continues it on the next line starting at the LEFT MARGIN rather than under the line it belongs to, so the continuation reads as a line of its own, and a player who copies what looks like a whole line loses the end of the list. Nothing in the tooltip says otherwise, so keeping both halves on one line each is the only place that hazard can be answered, and it is answered here rather than in a sentence on every field of every consumer.
...
Each of the two number settings is emitted with a description of its own: your `[mod-setting-description]` entry, then what 0 means beside a research dropdown, or the range it takes where there is no dropdown to defer to. The checker requires that entry for the same reason it requires one on a text setting. Beside a dropdown the sentence leads with the sentinel and not with the range, in the same shape as a text setting's switch line, because 0 there is not the bottom of a range a player might pick on purpose:

```
Leave this at 0 and the option chosen above supplies the number; otherwise a whole number up to 100000.
```

With no dropdown beside it there is no sentinel to explain and the sentence is the range alone, both bounds of it:

```
A whole number from 1 to 600.
```

The composed description of a cost dropdown names, after each tier's localised label and on the same line, the technology whose cost that tier copies, through the game's own `[technology-name]` entry for it (after `: cost of`), or `: the fallback cost` for a tier with no source. It stays on one line where an ingredient preset takes two, because a localised label followed by a technology name is one vocabulary and there is nothing in it to copy. A last line says that the pack text above or below it applies while it does not say `default`, exactly as an ingredient dropdown's does.

**A cost preset names its source technology through the game's locale key, with the internal name behind it.** The reference is `technology-name.<source>` with `<source>` itself as the fallback, so where the game defines that key the line shows the technology's displayed name and where it does not the line shows the internal name, with the rest of the tooltip unaffected either way. The base game composes the displayed name of a numbered technology instead of keying it, so `technology-name.logistics-2` is defined nowhere and a preset over that technology reads `logistics-2` on a line whose label reads `Logistics 2`. This is cosmetic, and it is not something you can fix by defining the key yourself: the locale namespace is flat and shared, so a `technology-name.logistics-2` entry in your own `.cfg` sets the displayed name of the base game's technology for every mod in the game. `CheckLocaleAdvisories` (`check_locale_advisories`), described under [Advisories, which are not findings](#advisories-which-are-not-findings), reports that hazard once per game key the library composes, and the entry is the one action that makes things worse. Leave the key alone.
