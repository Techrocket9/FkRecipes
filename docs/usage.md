...
A resolve-or-drop ingredient ladder that leaves the recipe with something to craft gets no such line, on purpose: it is what the ladder is for, and the library discloses it on the settings screen instead, in the ladder sentence it composes onto an ingredient dropdown's description and onto every text setting whose ladder nothing beside it already discloses. That sentence names all three things a ladder does, the mod's next name for an entry your mods lack, the entry left out where the mod offers nothing more, and two entries that land on one name having their amounts added, so a player reading the field knows the list the game builds can be shorter than the one the tooltip shows. Where the ladders run out on EVERY entry, the sentence stops being enough and the recipe carries a line of its own, the fifth in the block above: a recipe with no ingredients costs nothing to craft, which is a balance change nobody chose rather than a shorter list. Everything else in the block is presence, arithmetic or tree shape a player cannot check anywhere: a science pack that is no longer in a price, a research that now costs nothing, a cost copied from a technology whose own price could not be read, an amount capped at a ceiling, a prerequisite dropped to open a loop, and a research that hangs off nothing because no technology it takes its cost from has a cost this library can use.

WHERE THAT SETTINGS-SCREEN SENTENCE IS COMPOSED, and where it is not. It goes onto every ingredient dropdown, including one with no text setting beside it, whose whole composed description is then your own `[mod-setting-description]` entry and that one line. It goes onto a text setting unless the field beside it already says the same thing: an ingredient text bound beside an `IngredientsBy` dropdown does not carry it, because the lists the player is choosing between are that dropdown's presets and the dropdown's own description is where they are written out, while a text setting that stands alone and a science-pack text bound to a `CostFrom` beside a `CostBy` tier both do carry it. A cost dropdown carries none, because its presets render no list of internal names for a ladder to shorten, which is exactly why the science-pack field beside it keeps the sentence: it is the only one of that pair where a player can read the rule. What a copied cost's own packs do on a mod set that lacks them is a different fact and is disclosed where it happens, on the technology's own tooltip. A recipe declared with a plain `Ingredients` list and no setting at all has no field on the settings screen to carry it, so on that plan the ladder is disclosed to YOU, here, and nowhere a player looks. Declare an `IngredientsSetting` or an `IngredientsBy` dropdown if you want the player told; a recipe whose ladders empty it completely still says so on its own tooltip either way.
