...
v0.1.2 rewrites the text this library composes onto its own settings and adds three optional fields to the surface. No prototype this library builds out of your declaration moves, and nothing you already declare behaves differently: what moves is what a player reads on the Mod Settings screen, which was about 1050 characters per text field and is now about 400 to 550, plus three things you can now say that you could not.

- **A text setting's composed block is four or five lines instead of six.** The line saying that a list too long for one line continues on the next is gone from every composition, on a text setting and on an ingredient dropdown alike. What it disclosed is real, that the engine's own break leaves the continuation at the left margin so a player who copies what looks like a whole line loses the end of the list, and it is now documented for you, in [Ingredients the player writes](usage.md#ingredients-the-player-writes), rather than said on every field of every consumer.
- **The ladder line moved to the field that renders the lists it is about, and it is composed exactly once per pair.** A text setting carries it unless the dropdown beside it already says the same thing: an ingredient text bound beside an `IngredientsBy` dropdown does not, because that dropdown's own description carries the disclosure over the presets a player is choosing between. A text setting that stands alone carries it, whose own default line is then the list that applies, and so does a science-pack setting bound beside a `CostBy` tier, because a cost dropdown composes no ladder line at all and that field is the only one of the pair where a player can read the rule. What a copied cost's packs do on a mod set that lacks them is a different fact and is disclosed on the technology's own tooltip, where it happens.
- **The three ladder sentences are shorter.** They read `An entry your mods lack takes the mod's next name for it or is left out; two landing on one name are added, so what you craft can be shorter than shown.` on an ingredient text setting, `A pack your mods lack takes the mod's next name for it or is left out; two landing on one pack are added, so the research can take fewer packs than shown.` on a science-pack one, and the first with `so an option can craft a shorter list than it shows.` in place of its closing clause on an ingredient dropdown. Each keeps all three facts it always carried. A gate of yours that matches any of them moves.
...
Three additions to the surface come with it, and a plan that uses none of them is emitted byte for byte as it was:

- **`Describes` says which declaration a shared dropdown shows.** A bool on `IngredientChoices` and on `CostChoices` (`describes` in Rust). Where several declarations name one dropdown the library has always picked one, by declaration order, recipes then technologies with the last winning; that rule is unchanged where nothing is marked, so no existing plan moves, and it is now written down rather than left to be discovered. **In Rust this is a new field on two public structs**, so a suite that builds either with a struct literal and no `..Default::default()` stops compiling until it names it; in Go the zero value is the old behaviour and nothing moves. Two new plan-time refusals, both about your declaration alone: two marked declarations over one dropdown, and a marked `CostBy` with no `CostFrom` beside it, which composes nothing at all. A suite that asserts a plan is accepted sees it accepted; one that marks two declarations sees the first sentence.
- **`CostChoice.Display` says what a cost option costs in your own words.** A string (`display` in Rust), **also a new field on a public struct in Rust** with the same consequence for a struct literal. Where it is not empty the composed line reads `\n<label>: <your text>` in place of `: cost of <technology>` or `: the fallback cost`, and no `technology-name` key is composed for that choice, so `CheckLocaleAdvisories` loses the note about it. A suite that pins that report, or that matches the composed tail, moves for every choice you give one to.
- **`DescribeSetting` writes a setting's description from the plan.** It takes any of the six setting handles and a literal and stands in for that setting's `[mod-setting-description]` entry: a composition opens with the literal instead of your key, and a setting nothing is composed onto carries the literal alone. **`CheckLocale` changes with it**, which is the one place a consumer's suite can go red without touching anything: for a setting you describe inline the description entry stops being required, and an entry that is still in your `.cfg` is reported as `the [mod-setting-description] entry <name> is never shown; the plan describes that setting inline and the engine shows the plan's description instead`. A suite that asserts the report is empty goes red until you delete the entry. Nothing else in your locale file changes.

Apart from that one finding, no entry becomes required or stops being required, and `CheckLocale`'s findings are the ones they were.
