...
/// The one way a piece of the PLAYER's text reaches a message THIS MODULE
/// puts between quotation marks: every member of the invisible set becomes
/// its `U+XXXX` token, and a text with none of them is borrowed back
/// untouched. THIS MODULE IS THE SCOPE, not the crate: `data.rs`'s
/// `read_dropdown` quotes a stored dropdown value into `holds "..."` raw, and
/// it cannot call this one. See the NUL paragraph below.
///
...
/// VALUE is LENGTH-DEPENDENT (the row in agents/customizer-design.md): a
/// total of 22 bytes or fewer is cut at the byte between the file and the
/// guest and a load that COMPLETES persists the cut, while 23 to 48 reaches
/// the guest whole (measured whole over that range; nothing longer was read,
/// and the short-string signature is what says it holds above). So a pasted
/// NUL reaches this parser whenever the whole stored text is 23 bytes or
/// more. And the engine cuts a LOAD-FAILURE MESSAGE at a NUL, printing only
/// what precedes it. The two together are what this rule closes: an entry
/// carrying a NUL is quoted as `U+0000`, so no message built in THIS MODULE
/// can carry the byte out to be cut at. THE MODULE IS THE WHOLE OF "HERE":
/// `data.rs`'s `read_dropdown` quotes a stored dropdown value with no
/// escaper at all, and it cannot call this one, because a name of this
/// module reached from `data.rs` links the whole language into every
/// consumer (`quotable` is private, and the `pub(crate)` that would open it
/// is what `tests::source` refuses by name). It is an open item in
